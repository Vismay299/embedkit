        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {blogPosts.map((post) => (
            <Link
              key={post.id}
              href={`/blog/${post.slug}`}
              className="group block rounded-xl border border-gray-200 p-6 hover:shadow-lg hover:border-[#0D9488]/30 transition-all"
            >
              ...
              <h2 className="font-semibold text-gray-900 group-hover:text-[#0D9488] transition-colors">
                {post.title}
              </h2>
